...
$tsql = "SELECT COUNT(ClientID) FROM dbo.Clients where TypeOfClient = 1"; 
/* Execute the query. */ 
...
$tsql = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY ClientID)  AS RowNumber,  PhoneNumber,   Name, TinNumber, TypeOfClient  FROM dbo.Clients)  AS Temp WHERE (RowNumber BETWEEN ? AND ?) AND ([TypeOfClient] = 1)"; 
