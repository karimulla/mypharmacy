...
$tsql = "SELECT COUNT(Prod_GenricID) FROM dbo.Product_Generic"; 
/* Execute the query. */ 
...
$tsql = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY Prod_GenricID)  AS RowNumber, Prod_GenricID, GenricName,   GenricCode, GenricRemarks  FROM dbo.Product_Generic)  AS Temp WHERE RowNumber BETWEEN ? AND ?"; 
...
{ 
     $data_rows = $data_rows . "<tr><td> <a href='/update-generics.php?genricID=". $row[1] . "'>" . $row[2] . "</a></td> <td>" . $row[3] ." </td> <td>" . $row[4] . "</td> </tr>"; 
}
